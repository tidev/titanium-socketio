...
- (NSString *)on:(id)args {
    ...
    NSUUID *handlerId = [self.socket on:eventName callback:^(NSArray *data, SocketAckEmitter *ack) {
...
- (NSString *)once:(id)args {
    ...
    NSString *eventName = [TiUtils stringValue:[args objectAtIndex:0]];
    KrollCallback *callback = [args objectAtIndex:1];
    NSUUID *handlerId = [self.socket once:eventName callback:^(NSArray *data, SocketAckEmitter *ack) {
        // TODO: Handle ack callback
...
- (void)emit:(id)args {
    ...
    if (ackCallback != nil) {
        [[self.socket emitWithAck:eventName with:data] timingOutAfter:0 callback:^(NSArray *ackData) {
